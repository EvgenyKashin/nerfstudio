...
from nerfstudio.configs.base_config import MachineConfig, ViewerConfig
...
from nerfstudio.models.depth_instant_ngp import DepthInstantNGPModelConfig
...
from nerfstudio.pipelines.dynamic_dual_batch import DynamicDualBatchPipelineConfig
...
method_configs["instant-ngp-dual"] = TrainerConfig(
    method_name="instant-ngp-dual",
    steps_per_eval_batch=500,
    steps_per_save=2000,
    max_num_iterations=30000,
    mixed_precision=True,
    pipeline=DynamicDualBatchPipelineConfig(
        datamanager=DualDataManagerConfig(dataparser=InstantNGPDataParserConfig(), train_num_rays_per_batch=8192),
...
method_configs["nerfacto_dual"] = TrainerConfig(
    method_name="nerfacto_dual",
    steps_per_eval_image=1000,
    steps_per_save=2000,
    max_num_iterations=8000,
    mixed_precision=True,
    machine=MachineConfig(num_gpus=1),
    pipeline=VanillaPipelineConfig(
        datamanager=DualDataManagerConfig(
            dataparser=NerfstudioDataParserConfig(),
            train_num_rays_per_batch=4096,
            eval_num_rays_per_batch=4096,
            camera_optimizer=CameraOptimizerConfig(mode="off"),
...
method_configs["depth-instant-ngp-bounded"] = TrainerConfig(
    method_name="depth-instant-ngp-bounded",
    steps_per_eval_batch=500,
    steps_per_save=2000,
    max_num_iterations=30000,
    mixed_precision=True,
    pipeline=DynamicBatchPipelineConfig(
        target_num_samples=2**18,
