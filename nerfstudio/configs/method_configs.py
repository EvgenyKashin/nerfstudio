...
from nerfstudio.data.datamanagers.latent_datamanager import LatentDataManagerConfig
...
method_configs["nerfacto-latent"] = TrainerConfig(
    method_name="nerfacto-latent",
    steps_per_eval_batch=500,
    steps_per_save=2000,
    max_num_iterations=30000,
    mixed_precision=True,
    pipeline=VanillaPipelineConfig(
        datamanager=LatentDataManagerConfig(
            dataparser=LatentBlenderDataParserConfig(),
            train_num_rays_per_batch=4096,
            eval_num_rays_per_batch=4096,
            camera_optimizer=CameraOptimizerConfig(
                mode="SO3xR3",
